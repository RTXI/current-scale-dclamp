...
/***
  ...
  *
  * LivR2009.h, v1.0
  * Model used in Current Scaling Dynamic Clamp Plugin
  *
  * - Francis Ortega
  *
 ***/
...
#include <math.h>
#include "include/PowFast.hpp" // Optimized power function

/*** LivR2009 Class ***/
class LivR2009 {
	
public:
	LivR2009(void); // Constructor
	~LivR2009(void); // Destructor

	// Current enum (Remember to change constant "currentLength")
	enum current_t{
		IKS,
		INA,
		IKR,
	...
	current_t current;
	int currentLength; // equals 2
	
	void calc_currents(double, double, double); // Model current solver
	double get_current(current_t); // Return desired current
	double scale_current(current_t, double); // Return scaled current
	void reset(); // Reset model to initial conditions

private:
	double V;
	double dVdt;

	double fastEXP(double); // optimzed e^x function using PowFast class
